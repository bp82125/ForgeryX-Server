...
def extract_blocking_artifact(image, diff_threshold=50, accumulator_size=33, block_size=8):
    ...
    return cv2.normalize(block_artifact_map, None, 0, 1, cv2.NORM_MINMAX)


def compute_edge_artifacts(luminance, diff_threshold, accumulator_size):
    vertical_diff = compute_diff(luminance, diff_threshold, axis=0, accumulator_size=accumulator_size)
    horizontal_diff = compute_diff(luminance, diff_threshold, axis=1, accumulator_size=accumulator_size)
...
def process_edge_map(edge_diff, accumulator_size, axis):
    ...
    mid_filtered = cv2.medianBlur(summed_edges.astype(np.uint8), accumulator_size)
